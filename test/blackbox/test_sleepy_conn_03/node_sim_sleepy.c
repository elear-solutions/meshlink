...
static void meshlink_logger_cb(meshlink_handle_t *mesh, meshlink_log_level_t level,
    ...
    if(strcasestr(text, "Connection") || strstr(text, "reachable")) {
      if(strcasestr(text, "Node") && strcasestr(text, "became reachable")) {
        if(sscanf(text, "%*s %50s %*s %*s", node_name) != 1) {
          fprintf(stderr, "Got bad log text\n");
          return;
        }
        mesh_event_sock_send(clientId, NODE_REACHABLE, node_name, 50);
      }

      if(strcasestr(text, "Node") && strcasestr(text, "became unreachable")) {
        if(sscanf(text, "%*s %50s %*s %*s", node_name) != 1) {
          fprintf(stderr, "Got bad log text\n");
          return;
        }
        mesh_event_sock_send(clientId, NODE_UNREACHABLE, node_name, 50);
      }
