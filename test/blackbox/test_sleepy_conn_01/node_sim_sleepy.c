/*
    node_sim_sleepy.c -- Implementation of Node Simulation for Meshlink Testing
...
static bool gateway_reachable = false;
static int clientId;
...
static void node_status_cb(meshlink_handle_t *mesh, meshlink_node_t *node,
    ...
    if(!strcasecmp(node->name, "gateway")) {
      if(!reachable) {
        mesh_event_sock_send(clientId, NODE_UNREACHABLE, "gateway node become unreachable", 30);
      }
      else {
        fprintf(stderr, "GATEWAY is now reachable\n");
...
int main(int argc, char *argv[]) {
    ...
    while(!gateway_reachable) {
      fprintf(stderr, "gateway node still not reachable\n");
      sleep(1);
    }
    fprintf(stderr, "gateway node reachable\n");
