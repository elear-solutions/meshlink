...
static void meshlink_logger_cb(meshlink_handle_t *mesh, meshlink_log_level_t level,
    ...
    if(strstr(text, "Autodisconnecting")) {
      char *disconn_node = (char *)text + strlen("Autodisconnecting from ");
      mesh_event_sock_send(clientId, AUTO_DISCONN, disconn_node, strlen(disconn_node) + 1);
    }
    return;
}
